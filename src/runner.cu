...
void run_tensor_cores_double_buffering(int M, int N, int K, float alpha, __nv_bfloat16 *A, __nv_bfloat16 *B, float beta, float *C) {
    // The overall method can be separated into two steps:
    // 1) Loading data from global memory to shared memory --> similar to the previous kernel.
    // 2) Compute the dot product between elements --> where warptiling is implemented.
    constexpr uint NUM_THREADS{128};
    ...
    dim3 block_dim(NUM_THREADS);
    dim3 grid_dim(CEIL_DIV(N, BN), CEIL_DIV(M, BM));
    tensor_cores_double_buffering_gemm<NUM_THREADS, BM, BN, BK, WM, WN, WMMA_M, WMMA_N, WMMA_K>
        <<<grid_dim, block_dim>>>(M, N, K, alpha, A, B, beta, C);
}


void run_tensor_cores_two_level_pipeline(int M, int N, int K, float alpha, __nv_bfloat16 *A, __nv_bfloat16 *B, float beta, float *C) {
    // The overall method can be separated into two steps:
    // 1) Loading data from global memory to shared memory --> similar to the previous kernel.
    // 2) Compute the dot product between elements --> where warptiling is implemented.
    constexpr uint NUM_THREADS{128};
...
void run_tensor_cores_mma(int M, int N, int K, float alpha, __nv_bfloat16 *A, __nv_bfloat16 *B, float beta, float *C) {
    ...
    dim3 block_dim(NUM_THREADS);
    dim3 grid_dim(CEIL_DIV(N, BN), CEIL_DIV(M, BM));
    tensor_cores_mma_gemm<NUM_THREADS, BM, BN, BK, WM, WN>
        <<<grid_dim, block_dim>>>(M, N, K, alpha, A, B, beta, C);
}


void run_tensor_cores_mma_swizzled(int M, int N, int K, float alpha, __nv_bfloat16 *A, __nv_bfloat16 *B, float beta, float *C) {
    // The overall method can be separated into two steps:
    // 1) Loading data from global memory to shared memory --> similar to the previous kernel.
    // 2) Compute the dot product between elements --> where warptiling is implemented.
    constexpr uint NUM_THREADS{128};
    ...
    dim3 block_dim(NUM_THREADS);
    dim3 grid_dim(CEIL_DIV(N, BN), CEIL_DIV(M, BM));
    tensor_cores_mma_swizzled_gemm<NUM_THREADS, BM, BN, BK, WM, WN>
        <<<grid_dim, block_dim>>>(M, N, K, alpha, A, B, beta, C);
}


void run_tensor_cores_mma_three_level_pipeline(int M, int N, int K, float alpha, __nv_bfloat16 *A, __nv_bfloat16 *B, float beta, float *C) {
    // The overall method can be separated into two steps:
    // 1) Loading data from global memory to shared memory --> similar to the previous kernel.
    // 2) Compute the dot product between elements --> where warptiling is implemented.
    constexpr uint NUM_THREADS{128};
    ...
    dim3 block_dim(NUM_THREADS);
    dim3 grid_dim(CEIL_DIV(N, BN), CEIL_DIV(M, BM));
    tensor_cores_mma_three_level_pipeline_gemm<NUM_THREADS, BM, BN, BK, WM, WN>
        <<<grid_dim, block_dim>>>(M, N, K, alpha, A, B, beta, C);
}
...
void run_kernel(
    ...
    case 13:
        ...
        break;
    case 14:
        run_tensor_cores_mma_swizzled(M, N, K, alpha, A, B, beta, C);
        break;
    case 15:
        run_tensor_cores_mma_three_level_pipeline(M, N, K, alpha, A, B, beta, C);
        break;
    default:
        throw std::invalid_argument("Invalid kernel number.");
    }
